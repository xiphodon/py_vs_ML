#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2017/4/12 15:58
# @Author  : GuoChang
# @Site    : https://github.com/xiphodon
# @File    : decisiontree_regressor.py
...
def creat_data(n):
    ...
    '''
    np.random.seed(0)
    X = 5 * np.random.rand(n, 1)    # n行1列的随机数组
    y = np.sin(X).ravel()
    noise_num = (int)(n / 5)
...
def test_DecisionTreeRegressor(*data):
    ...
    X = np.arange(0.0, 5.0, 0.01)[:, np.newaxis]    #列方向增加一个维度
    Y = regr.predict(X)
    ax.scatter(X_train, y_train, label="train sample", c='g')
    ax.scatter(X_test, y_test, label="test sample", c='r')
    ax.plot(X, Y, label="predict_value", linewidth=2, alpha=0.5)    #回归曲线
    ax.set_xlabel("data")

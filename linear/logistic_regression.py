...
def load_data():
    ...
    return model_selection.train_test_split(X_train, y_train, test_size=0.25,
                                            random_state=0, stratify=y_train)  # stratify分层采样拆分成训练集和测试集，测试集大小为原始数据集大小的 1/4
...
def test_LogisticRegression(*data):
    ...
    print('Coefficients:%s, intercept %s' % (regr.coef_, regr.intercept_))  # regr.coef_ 权重向量 ， regr.intercept_ 截距b
